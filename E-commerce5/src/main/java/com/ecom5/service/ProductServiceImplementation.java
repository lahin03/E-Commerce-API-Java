...
import com.ecom5.model.Category;
import com.ecom5.model.Product;
import com.ecom5.repository.CategoryRepository;
...
public class ProductServiceImplementation implements ProductService {
    ...
    public Page<Product> getAllProduct(String category, List<String> colors, List<String> size,
        ...
        // Filter by sizes (using getSizes() which returns a Set<Size>)
        if (size != null && !size.isEmpty()) {
            products = products.stream()
                    .filter(p -> p.getSizes() != null && 
                                 size.stream().anyMatch(s -> p.getSizes().stream()
                                                             .anyMatch(sz -> sz.getName().equalsIgnoreCase(s))))
                    .collect(Collectors.toList());
        }
        ...
        List<Product> paginatedList = startIndex < products.size() ? products.subList(startIndex, endIndex) : List.of();
